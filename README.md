...
libgen.li has been down for a while and libgen.lol cetificates expired. Until this gets fixed, I recommend using thi script to generate (libgen.lol) link lists only, which can be downloaded with uget despite the certificate issue with libgen.lol (can't find a solution for aria2 yet). Call this script as usual, then open `.libgen_dl/link_lists/combined.txt` with uget batch download function.
